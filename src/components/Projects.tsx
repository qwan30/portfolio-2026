...
interface ProjectCardProps {
  project: ProjectItem;
  index: number;
  onExpandImage: (data: { url: string; title: string }) => void;
  onSelectProject: (project: ProjectItem) => void;
}

const ProjectCard = memo(({ project, index, onExpandImage, onSelectProject }: ProjectCardProps) => {
  const cardRef = useRef<HTMLElement | null>(null);
  const [activeImage, setActiveImage] = useState<string>(
    () => project.hoverPreviewImage || (project.gallery && project.gallery[0]?.url) || ''
  );
  ...
  const currentImage = activeImage || project.hoverPreviewImage || '';
  const activeGalleryItem = project.gallery?.find((g) => g.url === currentImage);

  return (
    <article 
      ref={cardRef}
      className="project-card reveal-init bg-white rounded-3xl md:rounded-[2.5rem] border border-black/10 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden w-full"
          ...
          <div className="p-4 rounded-2xl bg-gray-50/80 border border-black/5 flex flex-col gap-2 text-xs font-mono text-ink/80">
            <div className="flex items-center justify-between border-b border-black/5 pb-1.5">
              ...
              <span className="font-bold text-blue-700">
                {project.quickSpecs?.tech || project.techStack.slice(0, 2).join(' + ')}
              </span>
            </div>
            <div className="flex items-center justify-between border-b border-black/5 pb-1.5">
              <span className="text-ink/60">Hạ tầng:</span>
              <span className="font-bold text-ink">
                {project.quickSpecs?.infra || 'Docker & Cloud Native'}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-ink/60">Quy mô / Tải:</span>
              <span className="font-bold text-green-700">
                {project.quickSpecs?.scale || 'High Reliability'}
              </span>
            </div>
          </div>
...
const Projects = () => {
  ...
  const handleExpandImage = useCallback((data: { url: string; title: string }) => {
